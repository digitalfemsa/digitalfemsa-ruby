  ...
  class OrderResponseCheckout
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DigitalFemsa::OrderResponseCheckout` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DigitalFemsa::OrderResponseCheckout`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [allowed_payment_methods, can_not_expire, emails_sent, expires_at, failure_url, id, is_redirect_on_failure, livemode, metadata, name, needs_shipping_contact, object, on_demand_enabled, recurrent, slug, sms_sent, success_url, starts_at, status, type, url].hash
    end
